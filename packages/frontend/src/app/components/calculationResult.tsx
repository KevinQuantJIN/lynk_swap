...
const CalculationResult: React.FC<CalculationResultProps> = ({
  ...
  useEffect(() => {
    const fetchReserves = async () => {
      try {
        ...
        setReserves({
          amoy: {
            x: amoyReserves[1] as any as bigint,
            y: amoyReserves[0] as any as bigint,
          },
          base: {
            x: baseReserves[1] as any as bigint,
            y: baseReserves[0] as any as bigint,
          },
          fuji: {
            x: fujiReserves[1] as any as bigint,
            y: fujiReserves[0] as any as bigint,
          },
        });
      } catch (error) {
  ...
  useEffect(() => {
    if (amountIn && alpha !== null && beta !== null && gamma !== null) {
      const k = BigInt(Number(amountIn) * 1e18);
      ...
      const amoyAmount = cfmm(
        reserves.amoy.x,
        reserves.amoy.y,
        BigInt(alphaAmount)
      );
      const baseAmount = cfmm(
        reserves.base.x,
        reserves.base.y,
        BigInt(betaAmount)
      );
      const fujiAmount = cfmm(
        reserves.fuji.x,
        reserves.fuji.y,
        BigInt(gammaAmount)
      );
      ...
      setRouterInfo({
        amoy: {
          percentage: (alpha * 100).toFixed(2) + "%",
          amount: (Number(alphaAmount) / 1e18).toFixed(4),
          amount_out: (Number(amoyAmount) / 1e18).toFixed(10),
        },
        base: {
          percentage: (beta * 100).toFixed(2) + "%",
          amount: (Number(betaAmount) / 1e18).toFixed(4),
          amount_out: (Number(baseAmount) / 1e18).toFixed(10),
        },
        fuji: {
          percentage: (gamma * 100).toFixed(2) + "%",
          amount: (Number(gammaAmount) / 1e18).toFixed(4),
          amount_out: (Number(fujiAmount) / 1e18).toFixed(10),
        },
      });
    ...
    <div>
      {isLoading ? (
        <div className="flex justify-center items-center">
          ...
          {/* You can define a CSS loader for better UX */}
